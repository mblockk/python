    ...
    while len(fringe) > 0:
        ...
        if goal_test(current) == False:
            for i in range(0,len(child_states)):
                if not child_states[i][1] in closed:
                    closed.add(child_states[i][1])
                    fringe.append(child_states[i][1])
                    max_fringe = max(len(fringe), max_fringe)
        else:
            return current, states_expanded, max_fringe
...
def dfs(state):
    ...
    while len(fringe) > 0:
        ...
        if goal_test(current) == False:
            for i in range(0,len(child_states)):
                if child_states[i][1] not in closed:
                    closed.add(child_states[i][1])
                    fringe.append(child_states[i][1])
                    max_fringe = max(len(fringe), max_fringe)
        else:
...
def astar(state, heuristic):
    ...
    while len(fringe) > 0:
        ...
        if goal_test(current) == False:
            for i in range(0,len(child_states)):
                costs[child_states[i][1]] = costs[current]+1
                if child_states[i][1] not in closed:
                    if heuristic(child_states[i][1]) not in fringe:
                        cost = heuristic(child_states[i][1])
                        child = child_states[i][1]
